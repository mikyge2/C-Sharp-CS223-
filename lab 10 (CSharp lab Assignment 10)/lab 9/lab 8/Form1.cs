    ...
    public partial class Form1 : Form
    {
        ...
        private void add_Click(object sender, EventArgs e)
        {
            ...
            if (idtext.Text=="")
            {
                ...
                errorProvider1.SetError(idtext, "Field can't be empty");
            }
            else if(fntext.Text=="")
            {
                errorProvider1.Clear();
                errorProvider1.SetError(fntext, "Field can't be empty");
            }
            else if(lntext.Text=="")
            {
                errorProvider1.Clear();
                errorProvider1.SetError(lntext, "Field can't be empty");
            }
            else if (addresstext.Text == "")
            {
                errorProvider1.Clear();
                errorProvider1.SetError(addresstext, "Field can't be empty");
            }
            else if (male.Checked==false && female.Checked==false)
            {
                errorProvider1.Clear();
                errorProvider1.SetError(gendertext, "Field can't be empty");
            }
            else
            {
